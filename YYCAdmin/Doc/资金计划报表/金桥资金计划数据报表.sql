...
u8_balance AS (
    ...
    WHERE 
        T0.dr = 0
        <#if param('组织名称')?? && param('组织名称') != ''>
          AND T1.name = '${param('组织名称')}'
        </#if>
),
u8_balance_opening AS (
    ...
    FROM u8_balance u
    WHERE 
    <#if param('开始日期')?? && param('开始日期') != ''>
        u.cDate = DATE_FORMAT('${param('开始日期')}', '%Y-%m-01')
    <#else>
        u.cDate = DATE_FORMAT(CURDATE(), '%Y-%m-01')
    </#if>
),
u8_balance_closing AS (
    ...
    FROM u8_balance u
    WHERE 
    <#if param('结束日期')?? && param('结束日期') != ''>
        u.cDate = DATE_FORMAT('${param('结束日期')}', '%Y-%m-01')
    <#else>
        <#if param('开始日期')?? && param('开始日期') != ''>
            u.cDate = DATE_FORMAT('${param('开始日期')}', '%Y-%m-01')
        <#else>
...
plan_draw_base AS (
    ...
    WHERE
        ...
        AND T0.iDeleted = 0
        <#if param('开始日期')?? && param('开始日期') != ''>
          AND T0.beginDate >= '${param('开始日期')}'
        </#if>
        <#if param('结束日期')?? && param('结束日期') != ''>
          AND T0.endDate <= '${param('结束日期')}'
        </#if>
        <#if param('组织名称')?? && param('组织名称') != ''>
          AND T2.NAME = '${param('组织名称')}'
        </#if>
),
