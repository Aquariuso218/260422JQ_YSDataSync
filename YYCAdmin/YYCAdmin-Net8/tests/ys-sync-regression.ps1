...
Assert-SourceContains 'ZR.Service\Business\Ys\YsBillInstrumentFetchService.cs' 'NormalizeForInsert' 'instrument null normalization'
Assert-SourceContains 'ZR.Service\Business\YsBillSyncService.cs' 'IYsBillInstrumentFetchService' 'instrument fetch dependency wiring'
Assert-SourceContains 'ZR.Service\Business\YsBillSyncService.cs' '_ysBillInstrumentFetchService.SyncAsync' 'instrument sync routing'
Assert-SourceContains 'ZR.Service\Business\Ys\YsSettlementFetchService.cs' 'Deleteable<EF_MidYSBillData>()' 'refresh delete handling'
Assert-SourceContains 'ZR.Service\Business\Ys\YsSettlementFetchService.cs' 'x.TradetypeName != "贴现办理" && x.TradetypeName != "到期兑付" && x.TradetypeName != "银行托收"' 'refresh exclusion fix'
...
Assert-SourceContains 'ZR.Service\Business\Ys\YsConfigOptions.cs' 'YtenantId' 'instrument tenant config'
